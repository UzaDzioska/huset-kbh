let template = document.querySelector("#evttemp").content; //load the posts all at once
...
function fetchEvents() {
    ...
    fetch("http://zuzannadzialowska.com/wordpress/wp-json/wp/v2/events?_embed&per_page=10&categories=7,8,9,10&order=asc&page=" + page)
        .then(e => e.json())
        .then(showEvents);
}
...
function showSingleEvent(anEvent) {
    ...
    //get location
    clone.querySelector(".location").textContent = anEvent.acf.event_location;


    //get price and "free"
    if (anEvent.acf.event_price > 0) {
        clone.querySelector(".price span").textContent = anEvent.acf.event_price;
        clone.querySelector(".free").style.display = "none";
    } else {
        clone.querySelector(".price").style.display = "none";
    }
    ...
    // get time
    clone.querySelector(".time").textContent = anEvent.acf.event_time;



    //get category
    clone.querySelector(".cat").textContent = anEvent.categories;
    //get images
